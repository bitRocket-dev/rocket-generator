...
export const UIIcon: FC<Props> = memo(({ icon, size = 'md', dataCy = 'UIIcon' }): JSX.Element | null => {
  const { width, height } = utilityGetSizes(size);

  switch (icon) {
    case 'bank':
      return <Icon width={width} height={height} src={srcBank} data-cy={dataCy} />;
    case 'calendar':
      return <Icon width={width} height={height} src={srcCalendar} data-cy={dataCy} />;
    case 'close':
      return <Icon width={width} height={height} src={srcClose} data-cy={dataCy} />;
    case 'time':
      return <Icon width={width} height={height} src={srcTime} data-cy={dataCy} />;
    default:
      return null;
  }
});
