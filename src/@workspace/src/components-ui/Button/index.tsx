...
const Button = styled.button<PropsWrapper>`
  ...
  ${({ variant, theme, disabled }: PropsWrapper) =>
    variant === 'primary' &&
    `background-color: ${disabled ? theme.colors.light.inactive : theme.colors.dark.primary};
    color: ${theme.colors.light.secondary};
    `};
  &:hover {
    background-color: ${({ theme, disabled }: PropsWrapper) => (disabled ? 'grey' : theme.colors.dark.inactive)};
    color: ${({ theme, disabled }: PropsWrapper) => disabled && theme.colors.light.secondary};
  }
`;
...
export const UIButton: FC<Props> = memo(({ disabled, variant, onClick, icon, text, size = 'md' }): JSX.Element => {
